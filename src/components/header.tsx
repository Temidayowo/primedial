...
const Header = ({ theme, className, mobileClassName }: HeaderProps) => {
  ...
  return (
    <>
      ...
      */}
      <header className={cn("hidden lg:block w-full z-50 top-0 left-0 bg-transparent absolute", className)}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 md:px-8 lg:px-12">
          ...
          <nav className="space-x-5 font-clash-display font-medium">
            ...
            <Link href="/about" className={animatedLinkClasses}>About</Link>
            <Link href="/shop" className={animatedLinkClasses}>Shop</Link>
            <Link href="/projects" className={animatedLinkClasses}>Projects</Link>
            <Link href="/contact" className={animatedLinkClasses}>Contact</Link>
          </nav>
      ...
      {/* ===================================================== 
          MOBILE HEADER
          ===================================================== */}
      <header className={cn("block lg:hidden border-b border-gray-200 bg-white relative", mobileClassName)}>
          ...
          <div className="flex items-center space-x-5">
            ...
            >
              <div className="relative w-6 h-5">
                <span className={`absolute left-0 h-0.5 w-full bg-blue transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "top-2 rotate-45" : "top-0"}`}></span>
                <span className={`absolute left-0 top-2 h-0.5 w-full bg-blue transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "opacity-0 translate-x-5" : "opacity-100"}`}></span>
                <span className={`absolute left-0 h-0.5 w-full bg-blue transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "top-2 -rotate-45" : "top-4"}`}></span>
              </div>
            </button>
          </div>
        </div>
        ...
        >
          <nav className="flex flex-col space-y-4 px-6 py-6 font-clash-display font-medium text-blue">
            <Link href="/" onClick={closeMenu} className={animatedLinkClasses}>Home</Link>
            <Link href="/about" onClick={closeMenu} className={animatedLinkClasses}>About</Link>
            <Link href="/shop" onClick={closeMenu} className={animatedLinkClasses}>Shop</Link>
            <Link href="/projects" onClick={closeMenu} className={animatedLinkClasses}>Projects</Link>
            <Link href="/contact" onClick={closeMenu} className={animatedLinkClasses}>Contact</Link>
            <hr className="border-gray-100 my-2" />
            <button onClick={closeMenu} className="flex items-center space-x-3 pt-2 text-blue text-left">
