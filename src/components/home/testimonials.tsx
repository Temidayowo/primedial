...
const StarRating = ({ rating }: { rating: number }) => {
  return (
    <div className="flex gap-1 mb-4" role="img" aria-label={`Rated ${rating} out of 5`}>
      {[...Array(5)].map((_, i) => (
        <svg
          key={i}
...
// Testimonials are managed in /admin/testimonials.
            ...
            <StaggerItem key={testimonial.id} className="h-full">
              <div className="flex h-full flex-col justify-between bg-[#F8FAFC] p-8 rounded-2xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
                ...
                <div className="flex items-center gap-4 mt-auto border-t border-gray-200 pt-4">
                  <div className="h-10 w-10 shrink-0 flex items-center justify-center rounded-full bg-blue text-white font-bold font-clash-display">
